Feature: Reliance Jewels Website Navigation and Purchase Flow

Scenario: TestCase6 Comprehensive Navigation and Purchase Process

    Given the user has successfully navigated to the homepage
    When the user moves the cursor over the Rings section in the navigation bar
    And the user selects the Casual Wear option under the Gender category
    When the user navigates to the Categories section
    And the user chooses the Platinum category
    When the user looks for additional filters
    And the user applies the Try on Available filter
    And the user confirms the Try on Available option
    When the user picks the first product from the list
    Then the user proceeds to click on the Buy Now button
